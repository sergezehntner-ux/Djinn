(() => {
  ...
  function normalizeTask(t){return {place:'',repeatValue:0,repeatUnit:'none',plannedFor:'',blockedUntil:null,lastDoneAt:null,done:false,events:[],...t,events:Array.isArray(t.events)?t.events:[]};}
  function load(){
    try{
  ...
  function save(){rolloverTodayProgram();normalizeTodayProgram();activatePlannedTasks();localStorage.setItem(KEY,JSON.stringify(state));renderHeader();renderStats();renderBackupInfo();renderTodayProgram();}
  ...
  function isPlannedFuture(t){return !!(t.plannedFor && String(t.plannedFor)>todayKey());}
  function isAvailable(t){return !t.done && !isWaiting(t) && !isPlannedFuture(t);}
  ...
  function activatePlannedTasks(){normalizeTodayProgram();const today=todayKey();let changed=false;state.tasks.forEach(t=>{const planned=String(t.plannedFor||'').trim();if(!planned||planned>today)return;if(!inTodayProgram(t.id)){const e={taskId:t.id,status:'active',origin:'planned',addedAt:new Date().toISOString()};state.todayProgram.entries.push(e);recordTaskEvent(t,'planned',{origin:'planned',plannedFor:planned,automatic:true});}t.plannedFor='';changed=true;});return changed;}
  ...
  function taskStatus(t){if(t.done)return 'done';if(isWaiting(t)||isPlannedFuture(t))return 'waiting';return 'available';}
  ...
  function renderTasks(){
    ...
    box.innerHTML=tasks.map(t=>{
      const status=taskStatus(t);const statusText=isPlannedFuture(t)?`Prévue pour le ${formatDate(t.plannedFor+'T12:00:00')}`:status==='waiting'?`À reproposer après le ${formatDate(t.blockedUntil)}`:status==='done'?'Terminée':'Proposable maintenant';
      return `<article class="task-card ${status}" data-id="${t.id}"><div><h3>${t.done?'✓ ':''}${escapeHtml(t.name)}</h3><div class="task-meta"><span>≈ ${t.duration} min</span><span>Importance ${t.importance}/3</span><span>Urgence ${t.urgency}/3</span><span>Effort ${t.effort}/3</span>${t.place?`<span>📍 ${escapeHtml(t.place)}</span>`:''}${hasTemporalite(t)?`<span>↻ ${escapeHtml(temporaliteLabel(t))}</span>`:''}${t.plannedFor?`<span>📅 Prévoir le ${formatDate(t.plannedFor+'T12:00:00')}</span>`:''}</div><div class="task-status">${statusText}</div><p class="muted">${escapeHtml(t.consequence||'Aucune conséquence renseignée.')}</p></div><div class="task-actions"><button class="icon-btn history-task">Historique</button><button class="icon-btn edit-task">Modifier</button><button class="icon-btn delete-task">Supprimer</button></div></article>`;
    }).join('');
  ...
  function openForm(clear=true){$('taskForm').classList.remove('hidden');if(clear){returnToTaskId=null;$('formTitle').textContent='Ajouter une tâche';$('taskId').value='';$('taskName').value='';$('taskDuration').value=10;$('taskPlace').value='';$('taskImportance').value=2;$('taskUrgency').value=2;$('taskEffort').value=2;$('taskRepeatValue').value='0';$('taskRepeatUnit').value='none';previousRepeatUnit='none';updateTemporalSummary();$('taskPlannedFor').value='';$('taskConsequence').value='';}$('taskName').focus();}
  function closeForm(){$('taskForm').classList.add('hidden');returnToTaskId=null;}
  function editTask(id){const t=state.tasks.find(x=>x.id===id);if(!t)return;returnToTaskId=id;openForm(false);$('formTitle').textContent='Modifier la tâche';$('taskId').value=t.id;$('taskName').value=t.name;$('taskDuration').value=String(Math.min(60,Math.max(5,Math.round((Number(t.duration)||10)/5)*5)));$('taskPlace').value=t.place||'';$('taskImportance').value=t.importance;$('taskUrgency').value=t.urgency;$('taskEffort').value=t.effort;$('taskRepeatValue').value=t.repeatValue||'0';$('taskRepeatUnit').value=t.repeatUnit||'none';previousRepeatUnit=$('taskRepeatUnit').value;updateTemporalSummary();$('taskPlannedFor').value=t.plannedFor||'';$('taskConsequence').value=t.consequence||'';$('taskForm').scrollIntoView({behavior:'smooth',block:'start'});}
  function saveTask(){
    ...
    const data={name,duration:Math.max(1,Number($('taskDuration').value)||10),place:$('taskPlace').value.trim(),importance:Number($('taskImportance').value),urgency:Number($('taskUrgency').value),effort:Number($('taskEffort').value),repeatValue,repeatUnit,plannedFor:$('taskPlannedFor').value||'',consequence:$('taskConsequence').value.trim()};
  ...
  const eventLabels={done:'✓ Fait',accepted:'👍 Je m’y mets',refused:'☹ Pas envie',personal:'👤 Choix personnel',postponed:'↪ Repoussé',cancelled:'↶ Annulé',non_realized:'○ Non réalisé',planned:'📅 Prévu pour aujourd’hui'};
  ...
  function openTaskFromMind(id){
    const t=state.tasks.find(x=>x.id===id);if(!t)return;detailTaskId=id;const status=taskStatus(t);const statusText=isPlannedFuture(t)?`Prévue pour le ${formatDate(t.plannedFor+'T12:00:00')}`:status==='waiting'?`À reproposer après le ${formatDate(t.blockedUntil)}`:status==='done'?'Terminée':'Proposable maintenant';
  ...
  function renderLearning(){
    ...
    const label={done:'✓ Fait',accepted:'👍 Je m’y mets',refused:'☹ Pas envie',personal:'👤 Choix personnel',postponed:'↪ Repoussé',cancelled:'↶ Annulé',non_realized:'○ Non réalisé',planned:'📅 Prévu pour aujourd’hui'};
  ...
  const rolled=rolloverTodayProgram();const plannedActivated=activatePlannedTasks();if(rolled||plannedActivated)localStorage.setItem(KEY,JSON.stringify(state));renderHeader();renderBackupInfo();refreshSmartLists();renderTasks();renderLearning();renderStats();renderTodayProgram();chooseSuggestion();
